...
func (c *Controller) registBindInfo(ctx context.Context) error {
	sign, err := c.Config.Wallet.Sign(wallet.DefaultKeyName, []byte(c.args.KEY))
	if err != nil {
...
func (c *Controller) login(ctx context.Context) (string, error) {
	sign, err := c.Config.Wallet.Sign(wallet.DefaultKeyName, []byte(c.Config.AgentID))
	if err != nil {
		return "", err
	}
...
func (c *Controller) regist(ctx context.Context) error {
	pubKey, err := c.Config.Wallet.GetPubKey(wallet.DefaultKeyName)
	if err != nil {
		return err
	}

	encodedPubKey := base64.URLEncoding.EncodeToString(pubKey.Bytes())

	url := fmt.Sprintf("%s%s?node_id=%s&pub_key=%s", c.args.ServerURL, "/node/registWithWallet", c.Config.AgentID, url.QueryEscape(encodedPubKey))
